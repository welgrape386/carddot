    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        for (int i = 0; i < headers.length; i++) {
            ...
            while ((cols = reader.readNext()) != null) {
                ...
                if (cardId.matches("\\d+")) {
                    cardId = String.format("%05d", Integer.parseInt(cardId));
                }

                // 이미 존재하는 데이터인지 확인
                // benefit_group컬럼만 추가로 채움
                if (benefitRepository.existsById(benefitId)) {
                    Benefit existing = benefitRepository.findById(benefitId).orElse(null);
                    if (existing != null) {
                        existing.setBenefitGroup(getString(cols, headerMap, "benefit_group"));
                        benefitRepository.save(existing);
                    }
                    continue;
                }
            ...
            benefit.setPerformanceLevel(getString(cols, headerMap, "performance_level"));
            benefit.setUnitAmount(getIntOrNull(cols, headerMap, "unit_amount"));
            benefit.setUiTitle(getString(cols, headerMap, "ui_title"));
            benefit.setUiContent(getString(cols, headerMap, "ui_content"));
