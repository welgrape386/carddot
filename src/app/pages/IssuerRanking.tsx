    ...
    <div className="bg-[#F8FAFC] min-h-screen">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1280px] mx-auto px-6 py-8">
          ...
          <p className="text-gray-500 font-normal ml-9">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {issuers.map((issuer) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-normal transition-all flex items-center gap-1.5 ${
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-normal shadow-sm"
                ...
                <Link
                  ...
                  className="flex items-center gap-1 text-sm font-normal hover:underline transition-colors"
                ...
                {ranked.map((card, idx) => (
                  <Link key={card.id} to={`/cards/${card.id}`}>
                    <div className="px-6 py-5 flex items-center gap-5 hover:bg-gray-50/70 transition-all group">
                      {/* 순위 */}
                      <div
                        className={`flex-shrink-0 w-10 h-10 rounded-xl border-2 flex items-center justify-center text-lg font-normal ${medalBg[idx]}`}
                      >
                        {idx === 0 ? (
                          <span>{medals[0]}</span>
                        ) : (
                          <span
                            className={`text-sm font-normal ${medalColors[idx]}`}
                          >
                            {idx + 1}
                          </span>
                        )}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-[10px] font-normal px-1.5 py-0.5 rounded ${card.type === "credit" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"}`}
                          >
      ...
      <div className="max-w-[1280px] mx-auto px-6 pb-12">
        <div className="flex items-center gap-2 text-xs text-gray-400 font-normal justify-center">
          <CreditCard className="w-3.5 h-3.5" />
          랭킹은 카드고릴라 조회수, 발급수, 사용자 리뷰를 종합하여 산정되며,
          매월 1일 갱신됩니다.
        </div>
      </div>
    </div>
