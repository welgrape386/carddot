...
function SectionDivider({
  ...
  return (
    <div
      className={`flex items-center gap-2 px-6 py-3 border-t border-gray-100`}
...
export function CardComparison() {
  ...
  const allCategories = Array.from(
    new Set(selectedCards.flatMap((c) => c.benefits.map((b) => b.category))),
  );
  const commonCategories = allCategories.filter((cat) =>
    selectedCards.every((c) => c.benefits.some((b) => b.category === cat)),
  );
  const uniqueCategories = allCategories.filter(
    (cat) => !commonCategories.includes(cat),
  );

  const getBenefit = (card: Card, category: string) =>
    card.benefits.find((b) => b.category === category);
  const getBestValue = (
    category: string,
    field: "discountRate" | "maxMonthly",
  ) => {
    const vals = selectedCards.map(
      (c) => getBenefit(c, category)?.[field] || 0,
    );
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-8">
                  ...
                  {card ? (
                    <div className="border-2 border-[#1B3D7B]/20 rounded-2xl p-5 flex flex-col items-center gap-3 bg-[#1B3D7B]/2">
                      <button
                        ...
                        className="absolute top-2 right-2 w-6 h-6 rounded-full bg-gray-100 hover:bg-red-50 hover:text-red-500 text-gray-400 flex items-center justify-center transition-all"
                      >
                        <X className="w-3 h-3" />
                      </button>
                      <CardVisual card={card} size="md" />
                      <div className="text-center">
                        <div className="text-xs text-gray-400 mb-0.5 font-semibold">
                          {card.issuer}
                        </div>
                      ...
                      <button
                        ...
                        className="text-xs text-[#1B3D7B] font-bold hover:underline"
                      >
                        카드 변경
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full h-full min-h-44 border-2 border-dashed border-gray-200 hover:border-[#1B3D7B] rounded-2xl flex flex-col items-center justify-center gap-2 text-gray-400 hover:text-[#1B3D7B] transition-all"
                    >
        ...
        {selectedCards.length < 2 ? (
          <div className="text-center py-20">
            ...
            <Link
              to="/cards"
              className="mt-4 inline-flex items-center gap-1 text-sm text-[#1B3D7B] hover:underline font-bold"
            >
          ...
          <>
            ...
            >
              {selectedCards.map((card) => {
                ...
                if (card.minSpending === bestSpending)
                  pros.push("낮은 실적 조건");
                ...
                return (
                  <div
                    key={card.id}
                    className="bg-white rounded-xl border border-gray-100 px-4 py-3 flex flex-wrap gap-1"
                  >
                    {pros.length > 0 ? (
                      pros.map((p) => (
                        <span
                          key={p}
                          className="text-[10px] bg-[#1B3D7B]/8 text-[#1B3D7B] px-2 py-0.5 rounded-full font-bold"
                        >
                          🏆 {p}
                        </span>
                      ))
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
              ...
              <div className="flex border-b-2 border-gray-100 bg-gray-50/50">
                ...
                {selectedCards.map((card, idx) => (
                  <div
                    key={card.id}
                    className="flex-1 px-5 py-4 border-r border-gray-100 last:border-r-0"
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className="w-1.5 h-6 rounded-full"
                        style={{
                          backgroundColor: ["#1B3D7B", "#0ABFA3", "#F97316"][
                            idx
                          ],
                        }}
                      />
                      <div>
                        ...
                        <div className="text-xs font-black text-gray-800 leading-snug">
                          {card.name}
                        </div>
                      </div>
                    </div>
              ...
              {commonCategories.length > 0 && (
                <>
                  <SectionDivider
                    label="🔗 공통 혜택"
                    badge={`${commonCategories.length}개 항목`}
                    color="#f0fffe"
                  />
                  {commonCategories.map((cat, i) => {
                    ...
                    return (
                      <div
                        key={cat}
                        className={`flex border-t border-gray-50 ${i % 2 !== 0 ? "bg-gray-50/30" : ""}`}
                      >
                        ...
                        {selectedCards.map((card) => {
                          ...
                          if (!b)
                            return (
                              <div
                                key={card.id}
                                className="flex-1 px-5 py-4 border-r border-gray-50 last:border-r-0 flex items-center text-gray-300"
                              >
                          ...
                          return (
                            <div
                              key={card.id}
                              className={`flex-1 px-5 py-4 border-r border-gray-50 last:border-r-0 ${isTopRate ? "bg-teal-50/40" : ""}`}
                            >
                              <div className="flex items-center gap-2 mb-1.5">
                                <span
                                  className={`text-base font-black ${isTopRate ? "text-teal-600" : "text-gray-800"}`}
                                >
                                  {b.discountRate}%
                                </span>
                                {isTopRate && (
                                ...
                                <span
                                  className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${benefitTypeColor[b.type]}`}
                                >
                                  {benefitTypeLabel[b.type]}
                                </span>
                              </div>
                              <div className="text-xs text-gray-600 font-semibold">
                                월 최대{" "}
                                <span
                                  className={`font-black ${isTopMax ? "text-teal-600" : "text-gray-800"}`}
                                >
                              ...
                              <div className="text-[10px] text-gray-500 font-semibold mt-0.5 leading-tight">
                                {b.description}
                              </div>
                            </div>
                          );
              ...
              {[
                ...
                },
                {
                  ...
                  render: (card: Card) => (
                    <span
                      className={`text-xs font-bold px-2 py-1 rounded-lg ${card.type === "credit" ? "bg-blue-50 text-blue-700" : "bg-purple-50 text-purple-700"}`}
                    >
                ...
                },
                {
                  ...
                  render: (card: Card) => (
                    <div className="flex items-center gap-1.5">
                      {card.annualFee === bestFee && (
                        <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                      )}
                      <span
                        className={`text-sm font-black ${card.annualFee === 0 ? "text-green-600" : card.annualFee === bestFee ? "text-[#1B3D7B]" : "text-gray-800"}`}
                      >
                        {card.annualFee === 0
                          ? "무료"
                ...
                },
                {
                  ...
                  render: (card: Card) => (
                    <div className="flex items-center gap-1.5">
                      {card.minSpending === bestSpending && (
                        <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                      )}
                      <span
                        className={`text-sm font-black ${card.minSpending === 0 ? "text-green-600" : card.minSpending === bestSpending ? "text-[#1B3D7B]" : "text-gray-800"}`}
                      >
                        {card.minSpending === 0
                          ? "무실적"
                ...
                },
                {
                  ...
                  render: (card: Card) => (
                    <div className="flex items-center gap-1.5">
                      {card.maxBenefit === bestBenefit && (
                        <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                      )}
                      <span
                        className={`text-sm font-black ${card.maxBenefit === bestBenefit ? "text-[#1B3D7B]" : "text-gray-800"}`}
                      >
                  ...
                  render: (card: Card) => (
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span
                          className={`text-sm font-black ${card.score === bestScore ? "text-[#1B3D7B]" : "text-gray-800"}`}
                        >
                          {card.score}점
                        </span>
                      </div>
                ...
                },
                {
                  ...
                  render: (card: Card) => (
                    <div className="flex items-center gap-1.5">
                      {card.rating === bestRating && (
                        <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                      )}
                      <Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />
                      <span
                        className={`text-sm font-black ${card.rating === bestRating ? "text-[#1B3D7B]" : "text-gray-800"}`}
                      >
                        {card.rating}
                      </span>
                      <span className="text-xs text-gray-400 font-semibold">
                  ...
                  render: (card: Card) => (
                    <div className="space-y-1">
                      {card.eventBenefits.map((ev, i) => (
                        <div
                          key={i}
                          className="text-[11px] text-gray-600 font-semibold bg-orange-50 px-2 py-1 rounded-lg border border-orange-100"
                        >
                          {ev}
                        </div>
                      ))}
                  ...
                  render: (card: Card) => (
                    <div className="flex flex-wrap gap-1">
                      {card.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-bold"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                >
                  ...
                  {selectedCards.map((card) => (
                    <div
                      key={card.id}
                      className={`flex-1 px-5 py-4 border-r border-gray-50 last:border-r-0 flex items-center ${row.highlight(card) ? "bg-[#1B3D7B]/3" : ""}`}
                    >
                      {row.render(card)}
                    </div>
                  ))}
              ...
              {uniqueCategories.length > 0 && (
                <>
                  <SectionDivider
                    label="⭐ 차별화 혜택"
                    badge={`${uniqueCategories.length}개 항목`}
                    color="#fff8f3"
                  />
                  {uniqueCategories.map((cat, i) => {
                    const iconCard = selectedCards.find((c) =>
                      getBenefit(c, cat),
                    );
                    const icon = iconCard
                      ? getBenefit(iconCard, cat)?.icon
                      : "";
                    return (
                      <div
                        key={cat}
                      ...
                      >
                        ...
                        {selectedCards.map((card) => {
                          ...
                          return (
                            <div
                              key={card.id}
                              className={`flex-1 px-5 py-4 border-r border-gray-50 last:border-r-0 ${b ? "bg-orange-50/30" : ""}`}
                            >
                              {b ? (
                                <>
                                  <div className="flex items-center gap-2 mb-1.5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              >
                {selectedCards.map((card, idx) => {
                  const pros: string[] = [];
                  const cons: string[] = [];
                  if (card.annualFee === bestFee)
                    pros.push("연회비가 가장 낮음");
                  if (card.minSpending === bestSpending)
                    pros.push("전월실적 조건이 낮음");
                  if (card.maxBenefit === bestBenefit)
                    pros.push("월 혜택이 가장 큼");
                  if (card.rating === bestRating)
                    pros.push("사용자 만족도 최고");
                  if (card.score === bestScore) pros.push("혜택 점수 1위");
                  if (card.annualFee > bestFee)
                    cons.push(
                      `연회비 ${(card.annualFee - bestFee).toLocaleString()}원 더 비쌈`,
                    );
                  if (card.minSpending > bestSpending)
                    cons.push("전월실적 조건이 높음");
                  const accentColors = [
                    "border-[#1B3D7B]/30 bg-[#1B3D7B]/2",
                  ...
                  return (
                    <div
                      key={card.id}
                      className={`rounded-2xl border-2 p-5 ${accentColors[idx]}`}
                    >
                      <div className="text-[10px] text-gray-400 font-bold mb-0.5">
                        {card.issuer}
                      </div>
                      <div className="text-sm font-black text-gray-900 mb-4 leading-snug">
                        {card.name}
                      </div>
                      <div className="space-y-1.5 mb-3">
                        {pros.map((p) => (
                          <div
                            key={p}
                            className="flex items-start gap-1.5 text-xs text-green-700 font-semibold"
                          >
                      ...
                      {cons.length > 0 && (
                        <div className="space-y-1.5 mb-4">
                          {cons.map((c) => (
                            <div
                              key={c}
                              className="flex items-start gap-1.5 text-xs text-gray-500 font-semibold"
                            >
                      ...
                      <Link
                        to={`/cards/${card.id}`}
                        className="mt-2 w-full block text-center py-2 border-2 border-[#1B3D7B] text-[#1B3D7B] rounded-xl text-xs font-black hover:bg-[#1B3D7B] hover:text-white transition-all"
                      >
                        상세 보기
                      </Link>
                    </div>
