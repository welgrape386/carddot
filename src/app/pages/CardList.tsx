...
import {
  Star, Heart, GitCompare, X, Check,
  TrendingUp, TrendingDown, SlidersHorizontal,
  ChevronDown, ChevronUp, ArrowUp, ArrowUpDown,
} from 'lucide-react';
...
function getCategoryBenefit(
  ...
  const best = matched.reduce((a, b) => a.discountRate >= b.discountRate ? a : b);
  return { rate: best.discountRate, type: best.type };
}

function matchesCategories(card: typeof cards[0], selected: string[]): boolean {
...
const typeLabel: Record<string, string> = { discount: '할인', cashback: '캐시백', point: '적립', special: '' };
...
export function CardList() {
  ...
  const filteredCards = cards
    ...
    .filter(c => !selectedIssuers.length || selectedIssuers.includes(c.issuer))
    .filter(c => c.annualFee >= feeOption.min && c.annualFee <= feeOption.max)
    .filter(c =>
      spendingOption.label === '전체' ? true
      : spendingOption.label === '무실적' ? c.minSpending === 0
      : c.minSpending <= spendingOption.max,
    )
    .filter(c => !specialFilters.noSpending || c.minSpending === 0)
    .filter(c => !specialFilters.coupon || c.eventBenefits.some(e => e.includes('쿠폰') || e.includes('할인권') || e.includes('쿠폰') || e.includes('캐시백')))
    .sort((a, b) => {
  ...
  const specialFilterCount = Object.values(specialFilters).filter(Boolean).length;
  const hasDetailFilter =
    cardType !== 'all' || selectedIssuers.length > 0 ||
    feeOption.label !== '전체' || spendingOption.label !== '전체' || specialFilterCount > 0;
    ...
    <div className="bg-[#F8FAFC] min-h-screen">
        ...
        <div className="max-w-[1280px] mx-auto px-6">
          ...
          <div className="py-3 flex items-start gap-3">
            <div className="flex-shrink-0 pt-1.5 w-20">
              <span className="text-xs font-black text-gray-400 whitespace-nowrap">혜택 선택</span>
            </div>
              ...
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-black transition-all border ${
                  selectedCategories.length === 0
                    ? 'bg-[#4E40EF] text-white border-[#4E40EF]'
            ...
            >
              ...
              {hasDetailFilter && (
                <span className="bg-orange-500 text-white text-[9px] font-black px-1 py-0.5 rounded-full leading-none">
                  {[cardType !== 'all' ? 1 : 0, selectedIssuers.length, feeOption.label !== '전체' ? 1 : 0, spendingOption.label !== '전체' ? 1 : 0, specialFilterCount].reduce((a, b) => a + b, 0)}
                </span>
              )}
          ...
          >
            <div className="border-t border-gray-100 py-3 space-y-3">
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-gray-400 w-20 flex-shrink-0">카드 종류</span>
                  ...
                  {(['all', 'credit', 'debit'] as const).map(t => (
                    <button
                      ...
                      className={`px-4 py-1.5 text-xs font-black transition-all ${
                        cardType === t ? 'bg-[#4E40EF] text-white' : 'text-gray-500 hover:bg-gray-50'
                      }`}
                    >
              ...
              {/* 카드사 */}
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-gray-400 w-20 flex-shrink-0">카드사</span>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-black transition-all ${
                          selected
                            ? 'border-[#4E40EF] bg-[#4E40EF]/8 text-[#4E40EF]'
                            : 'border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50'
                        }`}
                      >
                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: issuer.color }} />
                        {issuer.short}
              ...
              {/* 연회비 */}
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-gray-400 w-20 flex-shrink-0">연회비</span>
                <div className="flex gap-1 flex-wrap">
                  {feeOptions.map(opt => (
                    <button key={opt.label} onClick={() => setFeeOption(opt)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-black transition-all ${feeOption.label === opt.label ? 'bg-[#4E40EF] text-white' : 'text-gray-500 hover:bg-gray-100 border border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              {/* 전월실적 */}
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-gray-400 w-20 flex-shrink-0">전월실적</span>
                <div className="flex gap-1">
                  {spendingOptions.map(opt => (
                    <button key={opt.label} onClick={() => setSpendingOption(opt)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-black transition-all ${spendingOption.label === opt.label ? 'bg-[#4E40EF] text-white' : 'text-gray-500 hover:bg-gray-100 border border-gray-200'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              {/* 특별 체크박스 필터 */}
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-gray-400 w-20 flex-shrink-0">추가 조건</span>
                <div className="flex flex-wrap gap-3">
                  {[
                    { key: 'coupon' as const,     label: '쿠폰 제공', desc: '호텔 숙박권 등' },
                    { key: 'noSpending' as const, label: '무실적',    desc: '할인, 적립' },
                    { key: 'voucher' as const,    label: '바우처',    desc: '' },
                  ].map(item => (
                    <button
                      key={item.key}
                      onClick={() => setSpecialFilters(prev => ({ ...prev, [item.key]: !prev[item.key] }))}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-black transition-all ${
                        specialFilters[item.key]
                          ? 'bg-[#4E40EF]/8 border-[#4E40EF] text-[#4E40EF]'
                          : 'border-gray-200 text-gray-600 hover:bg-gray-50'
                      }`}
                    >
                      <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        specialFilters[item.key] ? 'bg-[#4E40EF] border-[#4E40EF]' : 'border-gray-300'
                      }`}>
                        {specialFilters[item.key] && <Check className="w-2.5 h-2.5 text-white" />}
                      </div>
                      {item.label}
                      {item.desc && <span className="text-gray-400 font-bold">({item.desc})</span>}
                    </button>
                  ))}
                </div>
                {hasDetailFilter && (
                  <button
                    onClick={() => { setCardType('all'); setSelectedIssuers([]); setFeeOption(feeOptions[0]); setSpendingOption(spendingOptions[0]); setSpecialFilters({ coupon: false, noSpending: false, voucher: false }); }}
          ...
          <div className="border-t border-gray-100 py-2.5 flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 flex-1 flex-wrap">
              {sortOptions.map(opt => (
                ...
                >
                  ...
                  {opt.icon === 'none' && <SlidersHorizontal className="w-3 h-3" />}
                  {opt.label}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500 font-bold">
                총 <span className="text-[#4E40EF] font-black">{filteredCards.length}</span>개
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-6">
        ...
        {hasAnyFilter && (
          <div className="flex flex-wrap items-center gap-2 mb-5">
            ...
            {selectedCategories.map(cat => (
              <span key={cat} className="flex items-center gap-1 px-2.5 py-1 bg-[#4E40EF] text-white rounded-full text-xs font-black">
                {benefitCategories.find(c => c.key === cat)?.icon} {cat}
                <button onClick={() => toggleCategory(cat)} className="ml-0.5 opacity-70 hover:opacity-100">
                  <X className="w-3 h-3" />
                </button>
              </span>
            ))}
            {cardType !== 'all' && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                ...
                <button onClick={() => setCardType('all')}><X className="w-3 h-3" /></button>
              </span>
            )}
            {selectedIssuers.map(is => (
              <span key={is} className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                {is}
                <button onClick={() => toggleIssuer(is)}><X className="w-3 h-3" /></button>
              </span>
            ))}
            {feeOption.label !== '전체' && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                연회비 {feeOption.label}
                <button onClick={() => setFeeOption(feeOptions[0])}><X className="w-3 h-3" /></button>
              </span>
            )}
            {spendingOption.label !== '전체' && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                전월실적 {spendingOption.label}
                <button onClick={() => setSpendingOption(spendingOptions[0])}><X className="w-3 h-3" /></button>
              </span>
            )}
            {specialFilters.noSpending && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                무실적
                <button onClick={() => setSpecialFilters(p => ({ ...p, noSpending: false }))}><X className="w-3 h-3" /></button>
              </span>
            )}
            {specialFilters.coupon && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                쿠폰 제공
                <button onClick={() => setSpecialFilters(p => ({ ...p, coupon: false }))}><X className="w-3 h-3" /></button>
              </span>
            )}
            {specialFilters.voucher && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-full text-xs font-black">
                바우처
                <button onClick={() => setSpecialFilters(p => ({ ...p, voucher: false }))}><X className="w-3 h-3" /></button>
              </span>
            )}
          </div>
        )}
        ...
        {filteredCards.length > 0 ? (
          <div className="grid grid-cols-4 gap-4">
            {filteredCards.map(card => {
              const selectedBenefits = selectedCategories
                ...
                .filter(Boolean) as { cat: string; rate: number; type: string }[];
              ...
              return (
                <div key={card.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md hover:border-[#4E40EF]/20 transition-all group flex flex-col">
                  ...
                  <div className="bg-gray-50/70 p-5 flex items-start justify-between">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <div className="flex gap-1 flex-wrap justify-center">
                        <span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${card.type === 'credit' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>
                          {card.type === 'credit' ? '신용' : '체크'}
                        </span>
                        {card.tags.slice(0, 1).map(tag => (
                          <span key={tag} className="text-[9px] font-black bg-[#4E40EF]/8 text-[#4E40EF] px-1.5 py-0.5 rounded">{tag}</span>
                        ))}
                      </div>
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <button onClick={() => toggleFavorite(card.id)}
                        ...
                        <Heart className={`w-4 h-4 ${favorites.includes(card.id) ? 'fill-red-500' : ''}`} />
                      </button>
                      <button onClick={() => toggleCompare(card.id)}
                        className={`p-1.5 rounded-lg transition-all ${compareList.includes(card.id) ? 'text-[#4E40EF] bg-indigo-50' : 'text-gray-300 hover:text-gray-500'}`}>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <div className="text-[10px] text-gray-400 font-black mb-0.5">{card.issuer}</div>
                    <h3 className="text-sm font-black text-gray-900 mb-3 group-hover:text-[#4E40EF] transition-colors leading-snug">{card.name}</h3>

                    {/* ★ 선택 혜택 할인율 뱃지 */}
                    {selectedCategories.length > 0 && (
                      <div className="mb-3 rounded-xl border overflow-hidden">
                        {selectedBenefits.length > 0 && (
                          <div className="bg-[#4E40EF]/4 border-b border-[#4E40EF]/8 px-3 pt-2.5 pb-2">
                            <div className="text-[9px] text-[#4E40EF] font-black mb-1.5 uppercase tracking-wide">선택 혜택 할인율</div>
                            <div className="space-y-1.5">
                              {selectedBenefits.map(({ cat, rate, type }) => {
                                const catInfo = benefitCategories.find(c => c.key === cat);
                                return (
                                  <div key={cat} className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1">
                                      <span className="text-base font-black text-[#4E40EF] leading-none">{rate}%</span>
                                      <span className={`text-[9px] font-black px-1 py-0.5 rounded leading-none ${
                                        type === 'cashback' ? 'bg-green-50 text-green-600' :
                                        type === 'point'    ? 'bg-purple-50 text-purple-600' :
                              ...
                              return (
                                <div key={cat} className="flex items-center justify-between py-0.5">
                                  <div className="flex items-center gap-1 text-xs text-gray-400 font-bold">
                                    <span className="text-sm leading-none opacity-50">{catInfo?.icon}</span>
                                    <span>{cat}</span>
                                  </div>
                                  <span className="text-[10px] text-gray-300 font-black">혜택 없음</span>
                                </div>
                              );
                            })}
                          </div>
                    ...
                    <div className="space-y-1.5 mb-3">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-black ${card.annualFee === 0 ? 'text-green-600' : 'text-gray-900'}`}>
                          {card.annualFee === 0 ? '무료' : `${card.annualFee.toLocaleString()}원`}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-400 font-bold">전월실적</span>
                        <span className="font-black text-gray-900">
                          {card.minSpending === 0 ? '무실적' : `${(card.minSpending / 10000).toFixed(0)}만원`}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-400 font-bold">월 최대 혜택</span>
                        <span className="font-black text-[#4E40EF]">{(card.maxBenefit / 10000).toFixed(0)}만원</span>
                      </div>
                    </div>
                    ...
                    {selectedCategories.length === 0 && (
                      <div className="mb-3 pb-3 border-b border-gray-50">
                        <div className="text-[10px] text-gray-400 font-black mb-1.5">대표 혜택</div>
                        <div className="space-y-1">
                          {card.benefits.slice(0, 2).map((b, i) => (
                            <div key={i} className="flex items-center gap-1.5 text-xs text-gray-600 font-bold">
                              ...
                              <span className="font-black text-[#4E40EF]">{b.discountRate}%</span>
                              <span className="text-gray-400 text-[10px]">{b.type === 'cashback' ? '캐시백' : b.type === 'point' ? '적립' : '할인'}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {card.eventBenefits.length > 0 && (
                      <div className="mb-3">
                        <div className="bg-amber-50 border border-amber-100 rounded-lg px-2.5 py-1.5 flex items-start gap-1.5">
                          <span className="text-[10px] font-black text-amber-600 mt-0.5">🎉</span>
                          <span className="text-[10px] text-amber-700 font-bold leading-snug">{card.eventBenefits[0]}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-black text-gray-700">{card.rating}</span>
                        <span className="text-[10px] text-gray-400 font-bold">({card.reviews.toLocaleString()})</span>
                      </div>
                      <Link to={`/cards/${card.id}`}
                        className="text-xs px-3 py-1.5 text-white rounded-lg hover:opacity-90 transition-all font-black"
                        style={{ backgroundColor: '#4E40EF' }}>
                        상세보기
                      </Link>
                    </div>
                  </div>
          ...
          <div className="text-center py-24">
            ...
            <p className="text-gray-600 font-black mb-1">조건에 맞는 카드가 없어요</p>
            <p className="text-gray-400 text-sm font-bold mb-5">혜택 선택이나 상세 필터를 조정해보세요</p>
            <button onClick={resetAll} className="px-5 py-2.5 text-white rounded-xl text-sm font-black hover:opacity-90 transition-all" style={{ backgroundColor: '#4E40EF' }}>
              필터 전체 초기화
            </button>
          </div>
        )}
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
          <div className="rounded-2xl shadow-xl px-6 py-3.5 flex items-center gap-4" style={{ backgroundColor: '#4E40EF' }}>
            <div className="flex items-center gap-2">
              <GitCompare className="w-4 h-4 text-white" />
              <span className="text-white text-sm font-black">비교 중</span>
              <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full font-black">{compareList.length}/3</span>
            </div>
            <div className="flex gap-2">
              {compareList.map(id => {
                const card = cards.find(c => c.id === id);
                return card ? (
                  <div key={id} className="flex items-center gap-1 bg-white/10 text-white text-xs px-2 py-1 rounded-lg font-bold">
                    {card.name.split(' ').slice(-1)[0]}
                    <button onClick={() => toggleCompare(id)} className="ml-1 text-white/60 hover:text-white">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ) : null;
      ...
      <button
        ...
        style={{ bottom: compareList.length > 0 ? '88px' : '24px', backgroundColor: '#4E40EF' }}
        aria-label="맨 위로"
      >
