...
BASE_DIR      = "C:/Users/USER/claud_hd/kb"
FINAL_DIR     = f"{BASE_DIR}/claud/최종본"
EVENTS_JSON   = f"{BASE_DIR}/kb_events.json"
OUTPUT_CSV    = f"{FINAL_DIR}/kb_events.csv"
CARD_INFO_CSV = f"{FINAL_DIR}/kb_info.csv"
...
def load_card_meta():
    meta = {}
    with open(CARD_INFO_CSV, encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            cid = row.get("card_id", "").zfill(5)
            if cid:
                meta[cid] = {"card_name": row.get("card_name", "")}
